            ...
            if status == 0 and t_start < current_time:
                ...
                if self.machine_type == 'MentorPi_Acker':
                    ...
                    if twist.angular.z == 0:
                        ...
                        self.publish_servo_state([3], [1944])
                        time.sleep(0.2)
                        motor1 = MotorState()
                        motor1.id = 2
                        motor1.rps = 0.1
                        ...
                        self.publish_servo_state([3], [1056])
                        time.sleep(0.2)
                        motor1 = MotorState()
                        motor1.id = 2
                        motor1.rps = 1.0
                    ...
                    if self.machine_type == 'MentorPi_Tank':
                        if twist.angular.z == 0:
                            ...
                            t_start = current_time + 4.5 * math.pi / self.angular_speed
        ...
        if len(msg.points) >= 5:
            ...
            if 0 <= angle < 30:
                if sum(left_and_right) > 0:
                    if self.machine_type == 'MentorPi_Tank':
                        twist.angular.z = float(self.linear_speed) * 3
                else:
                    if self.machine_type == 'MentorPi_Tank':
                        twist.angular.z = float(-self.linear_speed) * 3
            elif 60 < angle <= 90:
