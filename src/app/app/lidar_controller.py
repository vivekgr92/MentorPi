                ...
                if self.running_mode == 1 and self.timestamp <= time.time():
                    ...
                    if len(min_dist_left_) > 1 and len(min_dist_right_) > 1:
                        ...
                        if self.last_act == 3: 
                            ...
                            twist.angular.z = -twist.linear.x/0.1 # # 右转转弯半径设为0.1m(set right turn radius to 0.1m)
                            self.last_act = 4
                            self.mecanum_pub.publish(twist)
                            ...
                            twist.linear.x = self.speed
                            twist.angular.z = -twist.linear.x/0.1 # 右转(turn right)
                            self.last_act = 1
                            self.mecanum_pub.publish(twist)
                            ...
                            if self.last_act == 5:
                                twist.linear.x = -0.3  # 后退(go backward)
                    ...
                    if len(dist_) > 1:
                        ...
                        if dist < 2 and abs(math.degrees(angle)) > 5:  # 控制左右(control the left and right)
                            if self.lidar_type != 'G4':
                                self.pid_yaw.update(-angle)
                                twist.angular.z = twist.linear.x*math.tan(common.set_range(self.pid_yaw.output, -0.698, 0.698))/0.145
                            elif self.lidar_type == 'G4':
                                self.pid_yaw.update(angle)
                                twist.angular.z = -twist.linear.x*math.tan(common.set_range(self.pid_yaw.output, -0.698, 0.698))/0.145
                        else:
