...
class JoystickController(Node):
    def __init__(self, name, num_robots):
        ...
        # Dynamically create the robot list(动态创建机器人列表)
        self.robots = []
    ...
    def axes_callback(self, axes):
        ...
        if self.machine == 'MentorPi_Mecanum':
            ...
            if steering_angle == 0:
                ...
                servo_state.id = [3]
                servo_state.position = [1500]  # Neutral position(中立位置)
        ...
        # Publish control messages to all robots(发布控制消息到所有机器人)
        for robot in self.robots:
    ...
    def start_callback(self, new_state):
        if new_state == ButtonState.Pressed:
            ...
            msg.off_time = 0.01
            msg.repeat = 1
            # Control the buzzers of all robots simultaneously(同时控制所有机器人的蜂鸣器)
            for robot in self.robots:
...
def main():
    # Specify the number of robots(指定机器人数量)
    num_robots = 3
