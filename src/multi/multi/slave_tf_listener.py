...
def qua2rpy(x, y, z, w):
    # Use quat2euler from transforms3d to get roll, pitch, and yaw(使用 transforms3d 的 quat2euler 获取 roll, pitch, yaw)
...
class SlaveTFListener(Node):
    def __init__(self):
        ...
        # Acquire the parameters(获取参数)
        self.declare_parameter('cmd_vel', '/robot_1/robot_controller/cmd_vel')
        self.declare_parameter('base_frame', 'robot_1/base_footprint')  # Remove leading slash(移除前导斜杠)
        self.declare_parameter('target_frame', 'point2')  # Remove leading slash(移除前导斜杠)

        cmd_vel = self.get_parameter('cmd_vel').get_parameter_value().string_value
        self.base_frame = self.get_parameter('base_frame').get_parameter_value().string_value.strip('/')  # Remove slash(去掉斜杠)
        self.target_frame = self.get_parameter('target_frame').get_parameter_value().string_value.strip('/')  # Remove slash(去掉斜杠)

        # Publisher(发布器)
        self.robot_vel_publisher = self.create_publisher(Twist, cmd_vel, 10)

        # TF2 listener(TF2 监听器)
    ...
    def update(self):
        msg = Twist()
        try:
            # Acquire the relative TF(获取相对的tf)
            now = rclpy.time.Time()
        ...
        # Extract the location info(提取位置信息)
        ...
        # Set the message and publish it(设置消息并发布)
...
def main(args=None):
    ...
    try:
        # Custom loop to replace rclpy.spin()(自定义循环，替代 rclpy.spin())
        while rclpy.ok():
            # Call the update method(调用 update 方法)
            node.update()
            # Process one callback (non-blocking)(处理一次回调，非阻塞)
