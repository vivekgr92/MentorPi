...
class TFFollower(Node):
    def __init__(self):
        ...
        self.declare_parameter('offset_x', -1.0)  # 1 meter behind robot1(在robot1的后方1米)
        ...
        # TF2 buffer and listener(TF2缓存和监听器)
        self.tf_buffer = tf2_ros.Buffer()
        self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)

        # TF2 broadcaster(TF2发布器)
    ...
    def timer_callback(self):
        try:
            # Get the latest pose of the leader (relative to the world coordinate frame)(获取leader的最新位姿（相对于世界坐标系）)
            leader_transform = self.tf_buffer.lookup_transform(
                'odom',  # target frame(目标帧)
                self.leader_frame,  # source frame(源帧)
                rclpy.time.Time())  # current time(当前时间)

            # Calculate the target pose of the follower(计算follower的目标位姿)
            ...
            # Apply the offset to the leader's pose(将偏移应用到leader的位姿)
            # Here, the offset is assumed relative to the leader's local coordinate frame(这里假设偏移是相对于leader的局部坐标系)
            # The offset needs to be transformed into the world coordinate frame(需要将偏移转换到世界坐标系)

            # First, calculate the offset coordinates in the world coordinate frame(先计算偏移在世界坐标系下的坐标)
            quat = leader_transform.transform.rotation
            siny_cosp = 2 * (quat.w * quat.z + quat.x * quat.y)
            cosy_cosp = 1 - 2 * (quat.y * quat.y + quat.z * quat.z)
            yaw = math.atan2(siny_cosp, cosy_cosp)

            # Calculate the offset position in the world coordinate frame(计算偏移后的世界坐标)
            ...
            # Keep the same orientation as the leader(保持与leader相同的姿态)
            follower_transform.transform.rotation = leader_transform.transform.rotation

            # Publish the follower's pose(发布follower的位姿)
