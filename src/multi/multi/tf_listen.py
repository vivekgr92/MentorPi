...
class SlaveTFListener(Node):
    def __init__(self):
        ...
        # Create the PID controller(创建 PID 控制器)
        self.pid_x = pid.PID(1.5, 0, 0)
        self.pid_y = pid.PID(1.5, 0, 0)
        self.pid_z = pid.PID(0.1, 0, 0)

        # Create a timer that calls the update function at regular intervals (创建定时器，每隔一段时间执行 update 函数)
        self.timer = self.create_timer(1.0 / 30.0, self.update)

    def update(self):
        msg = Twist()
        try:
            # Find relative transform (tf)(查找相对的 tf)
            now = rclpy.time.Time()
        ...
        # Extract the translation and rotation(提取平移和旋转)
        ...
        # Set message and publish (设置消息并发布)
...
def main(args=None):
    ...
    try:
        # Implement a custom loop as an alternative to spin()(自定义循环来替代 spin)
        while rclpy.ok():
